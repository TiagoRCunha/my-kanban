...
CREATE TABLE IF NOT EXISTS user_startup_columns (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    user_id BIGINT NOT NULL,
    title VARCHAR(50) NOT NULL,
    position INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_startup_columns_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT uq_startup_column UNIQUE (user_id, position)
);
...
CREATE TABLE IF NOT EXISTS user_custom_tags (
    ...
    position INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT fk_custom_tags_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    CONSTRAINT uq_custom_tag UNIQUE (user_id, position)
);

ALTER TABLE tasks ADD COLUMN IF NOT EXISTS tag_id BIGINT;

-- Only add constraint if it doesn't already exist (H2 workaround)
ALTER TABLE tasks DROP CONSTRAINT IF EXISTS fk_tasks_tag;
ALTER TABLE tasks ADD CONSTRAINT fk_tasks_tag FOREIGN KEY (tag_id) REFERENCES user_custom_tags(id) ON DELETE SET NULL;
